            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 mt-5">
                @foreach($products as $product)
                    <div class="max-w-md rounded overflow-hidden shadow-lg border-2 border-gray-300 flex justify-between flex-col">
                        ...
                        <div class="px-6 pt-4 flex lg:items-center lg:justify-between mb-2 flex-col lg:flex-row">
                        ...
                        <div class="px-6 pb-2 flex lg:items-center lg:justify-between mb-2 flex-col lg:flex-row">
                            @if($product->isActive === 1)
                            <form action="{{ route('setMenuNonActive', $product->id) }}" method="POST" class="{{ !Auth::check() ? 'hidden' : '' }}">
                                @method('patch')
                                @csrf
                                <button class="bg-red-500 hover:bg-red-900 text-white px-4 py-1 rounded-full">Non- Aktifkan</button>
                            </form>
                            @else
                            <form action="{{ route('setMenuActive', $product->id) }}" method="POST" class="{{ !Auth::check() ? 'hidden' : '' }}">
                                @method('patch')
                                @csrf
                                <button class="bg-green-500 hover:bg-green-900 text-white px-4 py-1 rounded-full">Aktifkan</button>
                            </form>
                            @endif
                        </div>
                    </div>
